...
def test_MCD_LCSE_16_view_daily_sales_details(the_licensee):
    ...
    expect(page.locator("text=/Sale\\s*details|Sales\\s*Details|Edit\\s*Sales|Edit\\s*sale/i").first).to_be_visible()
    expect(page.locator("text=/RM\\s*\\d+|Total\\s*Sales|Sales\\s*Amount/i").first).to_be_visible()
